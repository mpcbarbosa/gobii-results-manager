...
export async function completeTask(
  ...
  return adminFetch<{ success: boolean; taskId: string }>(
    `/api/admin/leads/${leadId}/tasks/complete`,
    {
      method: "PATCH",
...
export interface TasksInboxItem {
  task: {
    id: string;
    title: string;
    dueAt: string | null;
  ...
  isOverdue: boolean;
  dueBucket: "OVERDUE" | "TODAY" | "WEEK" | "LATER" | "NONE";
}

export interface TasksInboxResponse {
  success: boolean;
  count: number;
  items: TasksInboxItem[];
}

export async function fetchTasksInbox(): Promise<TasksInboxResponse> {
  return adminFetch<TasksInboxResponse>("/api/admin/tasks");
}

export async function rescheduleTask(
  taskId: string,
  dueAt: string | null,
): Promise<{ success: boolean; taskId: string; dueAt: string | null }> {
  return adminFetch(`/api/admin/tasks/${taskId}/reschedule`, {
    method: "PATCH",
